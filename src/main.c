...
void game_recompute_fov(rg_game_state* state)
{
    fov_map_compute(&state->fov_map,
                    state->entities.data[state->player].x,
                    state->entities.data[state->player].y,
                    state->fov_radius,
                    state->fov_light_walls);
}

void update(rg_app* app, SDL_Event* event, rg_game_state* state)
{
    ...
    if (state->recompute_fov) game_recompute_fov(state);

    if (action.type == ACTION_ESCAPE || action.type == ACTION_QUIT)
        app->running = false;
}

void draw(rg_app* app, rg_game_state* state)
{
    rg_map* game_map = &state->game_map;
    rg_fov_map* fov_map = &state->fov_map;
...
int main(int argc, char* argv[])
{
    ...
    tileset_create(
      &state.tileset, app.renderer, "res/dejavu10x10_gs_tc.png", 32, 8);

    terminal_create(&state.terminal,
                    &app,
                    state.screen_width,
                    state.screen_height,
                    &state.tileset,
    ...
    for (int y = 0; y < state.map_height; y++)
    {
        for (int x = 0; x < state.map_width; x++)
        {
            ...
            fov_map_set_props(
              &state.fov_map, x, y, !tile->block_sight, !tile->blocked);
        }
    }
